...
// Fetches latest videos via YouTube Data API when available, falling back to RSS.
// Always returns JSON list to keep the UI stable.
...
async function fetchPlaylistItems(playlistId, apiKey) {
  ...
  url.searchParams.set("maxResults", "50"); // grab up to 50 latest
  url.searchParams.set("key", apiKey);
